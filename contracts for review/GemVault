...
// GemVault is where you can lock xBling and get Bling as the reward.
contract GemVault is Ownable, ReentrancyGuard{
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    // Info of each user.
    struct UserInfo {
        uint256 amount; // How many LP tokens the user has provided.
        uint256 depositTime; // Timestamp of user deposit.
    ...
    // if users' balance fall below this threshold their timestamp will be reset
    uint256 immutable public MINIMUM_TO_RESET = 10 ** 18 * 100; 
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant() {
        ...
        if (user.amount > 0) {
            ...
            if (block.timestamp > user.depositTime + pool.lockDuration) {
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant() {
        ...
        require(block.timestamp > user.depositTime + pool.lockDuration, "Too soon");
